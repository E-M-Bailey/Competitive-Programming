...
inline constexpr ulli MOD = 1'000'000'007;
inline constexpr uli NLIM = 200'001;

inline vvulli precomp()
{
	vvulli PRE(10, vulli(NLIM, 1));
	loop(1, NLIM, i)
	{
		loop(0, 9, j)
		{
		...
		PRE[9][i] = (PRE[0][i - 1] + PRE[1][i - 1]) % MOD;
	}
	return PRE;
}

inline ulli go(const vvulli& PRE, const string& nStr, uli m)
{
	ulli went = 0;
	for (char ch : nStr)
	{

...
int main()
{
	...
	loop(0, n, i)
	{
		fDP.push_back(vulli(i + 1));
		rDP.push_back(vulli(i + 1));
	}
	loop(0, n, i)
	{
		fDP[i][i] = rDP[i][i] = A[i] * B[i];
	}
	loop(0, n - 1, i)
	{
		fDP[i + 1][i] = A[i] * B[i] + A[i + 1] * B[i + 1];
		rDP[i + 1][i] = A[i + 1] * B[i] + A[i] * B[i + 1];
	}
	loop(2, n, d)
	{
		loop(d, n, j)
		{
			uli i = j - d;
			fDP[j][i] = A[i] * B[i] + fDP[j - 1][i + 1] + A[j] * B[j];
			rDP[j][i] = A[j] * B[i] + rDP[j - 1][i + 1] + A[i] * B[j];
		}
	}

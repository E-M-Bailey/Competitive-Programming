...
inline constexpr uli rup2(uli x)
{
	x--;
	x |= x >> (1u << 0u);
	x |= x >> (1u << 1u);
...
void go()
{
	...
	// true iff l should be preserved
	static const auto comp = [&vec](uli l, uli r) -> bool
	{
		if (r == 0) return true;
		loop(l, r, i)
		{
			ci lc = vec[i % l], rc = vec[i];
			if (lc < rc)
				return true;
			else if (lc > rc)
				return false;
	...
	while (lens.size() > 1)
	{
		...
		loop(0, newM, i)
		{
			uli l = lens[i * 2], r = lens[i * 2 + 1];
			newLens[i] = comp(l, r) ? l : r;
		}
		lens = move(newLens);
	}

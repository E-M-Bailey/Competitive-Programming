...
//inline void merge(vuli& A, vbi& S, uli l, uli m, uli r)
//{
//	uli i = 0, j = 0, k = l , n1 = m + 1 - l, n2 = r - m;
//	vuli L(n1), R(n2);
//	copy_n(A.begin() + l, n1, L.begin());
//	copy_n(A.begin() + m + 1, n2, R.begin());
//	uli c = 0;
//	while (i < 0 && j < 0)
//}
//
//inline void msort(vuli& A, vbi& S, uli l, uli r)
//{
//	if (l < r)
//	{
//		uli m = l + (r - l) / 2;
...
void go()
{
	...
	{
		cin >> A[i].first;
		aLim = max(A[i].first + 1, aLim);
		A[i].second = i;
	}
	vuli CT(aLim, false);
	for (const puli& a : A) CT[a.first]++;
	const auto pred = [](const puli& l, const puli& r) -> bool
	{
		return l.first < r.first;
	};
	stable_sort(ALL(A), pred);
	bool correct = true;
	for (uli i = 0; i < n;)
	{
		puli a = A[i];
		uli ct = CT[a.first];
		if ((a.second + i) % 2 && ct % 2)
		{
			uli end = i + ct;
			uli even = 0, odd = 0;
			loop(i, end, j)
			{
				(j % 2 ? odd : even) += A[j].second;
			}
			if (even != odd)
			{
				correct = false;
...
//void qs(vuli& A, vbi& L, uli lo, uli hi)
//{
//	A
//}

//template<typename Iter>
//typedef vpuli::iterator Iter;
//inline void psort(Iter first, Iter last)
//{
...
//				if (l <= r)
//				{
//					//l.second ^= ((lItr - lo) + (aItr - aux.begin())) % 2;
//					
//					*aItr = move(l);
//					aItr++;
//					lItr++;
...
//				else
//				{
//					//r.second ^= ((rItr - lo) + (aItr - aux.begin())) % 2;
//					*aItr = move(r);
//					aItr++;
//					rItr++;
//					c++;
...
//	uli n;
//	cin >> n;
//	vector<pair<uli, bi>> A(n);
//	uli aLim = 0;
//	loop(0, n, i)
//	{
//		cin >> A[i].first;
//		A[i].second = false;
//		aLim = max(aLim, A[i].first--);
//	}
//	bool res;
//	if (n == 1)
//		res = true;
...
//	{
//		psort(ALL(A));
//		//size_t inv = mergeSortInversionCount(ALL(A));
//		res = true;
//		uli cur = MAX(uli);
//		uli numL = 0;
//		uli oddL = 0, evenL = 0;
//		for (const pair<uli, bool>& a : A)
//		{
//			if (a.first != cur)
//			{
//				if (numL % 2 == 1 || oddL != evenL)
//				{
//					res = false;
//					break;
//				}

...
typedef array<uli, 6> Node;

array<array<uli, 3>, 6> Perms
{
	array<uli, 3>{0, 1, 2},
	array<uli, 3>{0, 2, 1},
	array<uli, 3>{1, 0, 2},
...
void go()
{
	...
	loop(0, n, i)
	{
		...
		cin >> ch;
		uli a = ch - 'a';
		loop(0, 6, j) A[i][j] = Perms[j][i % 3] != a;
	}

	const auto op = [&A](const Node& left, const Node& right, size_t lo, size_t mid, size_t hi) -> Node
	{
		Node ret;
		loop(0, 6, j) ret[j] = left[j] + right[j];
		return ret;
	};

	segment_tree<Node, decltype(op)> st(A, Node{ 0, 0, 0, 0, 0, 0 }, op);

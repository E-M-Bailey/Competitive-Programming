...
constexpr uli MAXK = 18;
constexpr uli MAXN = (1u << MAXK) - 1;
//char ARR[MAXN];
...
void go()
{
	...
	loop(0, n, i)
	{
		...
		{
		case '0':
			node.f = node.l ? 1 : tree[idx * 2 + 1].f;
			break;
		case '1':
			node.f = node.l ? 1 : tree[idx * 2 + 2].f;
			break;
		case '?':
			node.f = node.l ? 2 : tree[idx * 2 + 1].f + tree[idx * 2 + 2].f;
			break;
		}
	}
	...
	loop(0, Q, q)
	{
		...
		if (c != node->c)
		{
			...
			while (true)
			{
				...
				{
				case '0':
					newF = node->l ? 1 : tree[idx * 2 + 1].f;
					break;
				case '1':
					newF = node->l ? 1 : tree[idx * 2 + 2].f;
					break;
				case '?':
					newF = node->l ? 2 : tree[idx * 2 + 1].f + tree[idx * 2 + 2].f;
					break;
				}
				if (newF == node->f) break;
				node->f = newF;
				if (idx == 0) break;
				idx = (idx - 1) / 2;
				node = &tree[idx];

...
int main()
{
	...
	tCases()
	{
		...
		uli eMin = C[0], oMin = C[1];
		ulli eSum = C[0], oSum = C[1], minCost = n * ulli(C[0] + C[1]);
		loop(2, n, i)
		{
			uli c = C[i];
			if (i % 2 == 0)
			...
			minCost = min(minCost, eSum + oSum + eMin * ulli(n - (i + 2) / 2) + oMin * ulli(n - (i + 1) / 2));
		}

		cout << minCost << endl;

		//const uli E = (n + 1) / 2, O = n / 2;
		//vuli even(E), odd(O);
		//loop(0, n, i)
		//	cin >> (i % 2 == 0 ? even : odd)[i / 2];
		//uli mCE = even[0], mCO = odd[0];
		//loop(1, E, i)
		//{
		//	uli c = even[i];
		//	if (c < mCE)
		...
		//ulli cost = ulli(mCE) * O + ulli(mCO) * E;
		//loop(0, E, i)
		//	cost += even[i];
		//loop(0, O, i)
		//	cost += odd[i];

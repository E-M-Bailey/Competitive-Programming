...
constexpr uli MAXN = 1000001;
vuli LP(MAXN + 1, 0);
vuli DLP(MAXN + 1, 0);
vbi ISP(MAXN + 1, false);
vuli PR;

void sieve()
{
	PR.reserve(MAXN);
	for (uli i = 2; i <= MAXN; i++)
	{
		if (LP[i] == 0)
		{
			LP[i] = i;
		...
		for (uli j = 0; j < PR.size() && PR[j] <= LP[i] && i * PR[j] <= MAXN; j++)
		{
			LP[i * PR[j]] = PR[j];
			DLP[i * PR[j]] = i;
		}
...
void go()
{
	...
	vuli firstIdx(maxA, MAX(uli));
	vuli maxGap(maxA, MAX(uli));
	vuli prevIdx(maxA, MAX(uli));
}
...
//inline constexpr uli lsBit(uli x)
//{
//	return x & -x;
//}
//
...
struct SegTree
{
	...
	inline SegTree(uli n, Op op, T id);
	//template<typename Iter>
	//inline SegTree(Iter first, uli n, Op op, T id);
	inline void build(const vector<T>& vec, uli tLo, uli tHi, uli v)
	{
		if (tLo == tHi)
			t[v] = vec[tLo];
		else
		{
			uli tm = (tLo + tHi) / 2;
			build(vec, tLo, tm, v * 2 + 1);
			build(vec, tm + 1, tHi, v * 2 + 2);
			uli lSize = tm + 1 - tLo;
			uli rSize = tHi - tm;
			t[v] = op(t[v * 2 + 1], t[v * 2 + 2], lSize, rSize);
		}
	}
	inline SegTree(const vector<T>& vec, Op op, T id) :
		SegTree(vec.size(), op, id)
	{
		build(vec, 0, n - 1, 0);
	}

	//template<typename Iter>
	//inline void build(Iter& itr, uli n, uli v = 0);


	inline T sum(uli lo, uli hi, uli tLo, uli tHi, uli v);
	inline T sum(uli lo, uli hi) { return sum(lo, hi, 0, n - 1, 0); }
	inline void update(uli idx, const T& val, uli tLo, uli tHi, uli v);
	inline void update(uli idx, const T& val) { update(idx, val, 0, n - 1, 0); }
};

template<typename T, typename Op>
inline SegTree<T, Op>::SegTree(uli n, Op op, T id) :
	n(n),
	op(op),
	id(id),
	t(new T[n - 1])
...
template<typename T, typename Op>
inline T SegTree<T, Op>::sum(uli lo, uli hi, uli tLo, uli tHi, uli v)
{
	if (lo > hi || lo > tHi || tLo > hi)
		return id;
	if (lo == tLo && hi == tHi)
		return t[v];
	uli tm = (tLo + tHi) / 2;
	uli lSize = lo > min(hi, tm) ? 0 : min(hi, tm) - lo;
	uli rSize = min(lo, tm + 1) > hi ? 0 : hi - min(lo, tm + 1);
	T lSum = sum(lo, min(hi, tm), tLo, tm, v * 2 + 1);
	T rSum = sum(max(lo, tm + 1), hi, tm + 1, tHi, v * 2 + 2);
	return op(lSum, rSum, lSize, rSize);
}

template<typename T, typename Op>
inline void SegTree<T, Op>::update(uli idx, const T& val, uli tLo, uli tHi, uli v)
{
	if (tLo == tHi)
	{
		t[v] = val;
	...
	uli tm = (tLo + tHi) / 2;
	if (idx <= tm)
		update(idx, val, tLo, tm, v * 2 + 1);
	else
		update(idx, val, tm + 1, tHi, v * 2 + 2);
	uli n = tHi + 1 - tLo;
	t[v] = op(t[2 * v + 1], t[2 * v + 2], n / 2, (n + 1) / 2);
}
...
struct stElem
{
	...
	inline constexpr stElem(lli sum, lli min, lli argmin) :
		sum(sum),
		min(min),
		argmin(argmin)
	{}

	inline constexpr stElem(lli val) :
		stElem(val, val, 0)
	{}

	inline stElem() {}
};
constexpr stElem stId{ 0, MAX(lli), 0 };
...
void go()
{
	...
	const auto stOp = [](const stElem& l, const stElem& r, uli lSize, uli rSize) -> stElem
	{
		lli s = l.sum + r.sum;
		lli m = min(l.min, r.min);
		uli a = l.min <= r.min ? l.argmin : lSize + r.argmin;
		return stElem{ s, m, a };
	};

	SegTree ST(A, stOp, stId);

	uli k = n;

	loop(0, n, i)
	{
		stElem prefix = ST.sum(0, i);
		while (prefix.sum < 0)
		{
			ST.update(prefix.argmin, 0ll);
			k--;
			prefix = ST.sum(0, i);
		}
	}

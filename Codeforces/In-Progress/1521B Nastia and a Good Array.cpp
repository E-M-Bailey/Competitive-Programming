...
{
	uli i, j, x, y;
	inline constexpr Operation(uli i, uli j, uli x, uli y) : i(i), j(j), x(x), y(y) {}
};
...
void go()
{
	...
	loop(0, T, t)
	{
		...
		{
			uli i = A[0] > A[1] ? 1 : 0; // smol
			uli j = 1 - i;
			if (gcd(A[0], A[1]) > 1)
			{
				A[j] = A[i] + 1;
				ops.emplace_back(0, 1, A[0], A[1]);
			}
		}
		//for (; A.end() - itr > 3; itr++)
		for (uli i = 0; i + 2 < n; i++)
		{
			...
			ops.emplace_back(j, k, y, z);
		}
		cout << ops.size() << endl;
		for (const Operation& op : ops)
		{
			writed(make_tuple(op.i + 1, op.j + 1, op.x, op.y), ' ');
			cout << endl;
		}
	}
}
...
void go()
{
...
#if VERSION == 2
	const auto L = [](uli i, uli j) -> char { return i % 5 * 5 + j % 5 + 'a'; };
	vvci G(n, vci(m));
#endif
	bool S;
	if (n % 2 == 0)
	{
		...
		if (S)
		{
			loop(0, n / 2, i) loop(0, m, j)
			{
				G[2 * i][j] = G[2 * i + 1][j] = L(2 * i, j);
			}
			loop(0, m / 2, j)
			{
				loop(0, n, i)
	...
	else
	{
		...
		if (S)
		{
			loop(0, n / 2, i) loop(0, m, j)
			{
				G[2 * i + 1][j] = G[2 * i + 2][j] = L(2 * i + 1, j);
			}
			loop(0, m / 2, j)
			{
				G[0][2 * j] = G[0][2 * j + 1] = L(0, 2 * j);
			}
			loop(0, m / 2, j)
			{
				loop(1, n, i)
				{
					if (i - 1 + j * (n - 1) >= k - m / 2) break;
					G[i][2 * j] = G[i][2 * j + 1] = L(i, 2 * j);
				}
			}
		}
	...
	if (S)
	{
		...
		{
			vvci newG(m, vci(n));
			loop(0, n, i) loop(0, m, j) newG[j][i] = G[i][j];
			G = move(newG);
			swap(m, n);
		}
		loop(0, n, i)
		{
			loop(0, m, j) cout << G[i][j];
			cout << endl;
		}
	}
#endif

...
int main()
{
	...
	loop(0, T, t)
	{
		...
		const auto cmpH = [](puli lhs, puli rhs)
		{
			return lhs.first < rhs.first;
		};
		const auto cmpQ = [](puli lhs, puli rhs)
		{
			return lhs.first > rhs.first;
		};
		sort(H.begin(), H.end(), cmpH);
		priority_queue<puli, vpuli, decltype(cmpQ)> Q(cmpQ);
		vuli S(n, 0);
		loop(0, m, i)
		{
			puli h = H.back();
		...
		loop(m, n, i)
		{
			...
			puli h = H.back();
			H.pop_back();
			Q.emplace(q.first + h.first, q.second);
			S[h.second] = q.second + 1;
		}
		cout << "YES" << endl;
		loop(0, n, i)
		{

...
void go()
{
	...
	loop(0, k, i) cin >> T[i];
	vuli tPerm(k);
	const auto aPred = [&A](uli l, uli r) -> bool
	{
		return A[l] < A[r];
	};
	const auto tPred = [&T](uli l, uli r) -> bool
	{
		return T[l] < T[r];
	};
	iota(ALL(tPerm), 0);
	set<uli, decltype(aPred)> aSet(aPred);
	sort(ALL(tPerm), tPred);
	for (uli i : tPerm)
	{
		decltype(aSet)::const_iterator next = aSet.lower_bound(i);
		decltype(aSet)::const_iterator prev;
		if (next == aSet.cbegin()) prev = aSet.cend();
		else
		{
			prev = next;
			prev--;
		}
		if ((prev == aSet.cend() || T[i] < T[*prev] + A[i] - A[*prev]) && (next == aSet.cend() || T[i] < T[*next] + A[*next] - A[i]))
			aSet.insert(i);
	}
	vuli R(n);
	uli first = *aSet.cbegin(), last = *aSet.crbegin();
	loop(0, A[first], a)
		R[a] = T[first] + A[first] - a;
	loop(A[last], n, a)
		R[a] = T[last] + a - A[last];
	decltype(aSet)::const_iterator lo = aSet.cbegin(), hi = aSet.cbegin();
	hi++;
	for (; hi != aSet.cend(); lo++, hi++)
	{
		uli aLo = A[*lo], aHi = A[*hi], tLo = T[*lo], tHi = T[*hi];
		loop(aLo, aHi, a) R[a] = min(tLo + a - aLo, tHi + aHi - a);
	}
	loop(0, n, a) cout << R[a] << ' ';
	cout << endl;
}
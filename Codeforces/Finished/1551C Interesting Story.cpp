...
struct Word
{
	array<li, 5> vals{ 0, 0, 0, 0, 0 };
	array<li, 5> diffs{ 0, 0, 0, 0, 0 };
};
...
void go()
{
	...
	vector<Word> W(n);
	{
		...
		loop(0, n, i)
		{
			...
			for (char ch : str)
				W[i].vals[ch - 'a']++;
			loop(0, 5, j) loop(0, 5, k)
			{
				if (j == k)
					W[i].diffs[j] += W[i].vals[k];
				else
					W[i].diffs[j] -= W[i].vals[k];
			}
	...
	loop(0, 5, i)
	{
		sort(ALL(P), [&W, i](uli l, uli r) { return W[l].diffs[i] > W[r].diffs[i]; });
		li D = 0;
		uli c = 0;
		for (uli p : P)
		{
			li newD = D + W[p].diffs[i];
			if (newD > 0)
			{
				D = newD;
				c++;

...
void go()
{
	uli n;
	cin >> n;
	vector<array<uli, 5>> R(n);
	loop(0, n, i) loop(0, 5, j) cin >> R[i][j];

	const auto cmp = [&R](uli l, uli r) -> bool
	{
		if (r == MAX(uli)) return true;
		if (l == MAX(uli)) return false;
		uli num = 0;
		loop(0, 5, j) num += R[l][j] < R[r][j];
		return num >= 3;
	};

	const auto op = [&cmp, n](uli l, uli r, size_t lo, size_t mid, size_t hi) -> uli
	{
		if (l == MAX(uli) && r == MAX(uli)) return MAX(uli);
		if (cmp(l, r))
		{
			loop(mid, min(n, (uli)hi), i) if (!cmp(l, i)) return MAX(uli);
			return l;
		}
		else
		{
			loop(lo, min(n, (uli)mid), i) if (!cmp(r, i)) return MAX(uli);
			return r;
		}
	};
	...
	vuli I(n);
	iota(ALL(I), 0);
	segment_tree<uli, decltype(op)> ST(I, MAX(uli), op);
	uli res = ST[range(0, n)];
	if (res == MAX(uli)) cout << -1 << endl;
	else cout << res + 1 << endl;

	//vuli A(n);
	//iota(ALL(A), 0);
	//sort(ALL(A), cmp);
	//if (n == 1 || cmp(A[0], A[1])) cout << A[0] + 1;
	//else cout << -1;
	//cout << endl;
}
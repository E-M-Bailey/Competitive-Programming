...
void go()
{
	string s, t;
	cin >> s >> t;
	uli n = s.size(), m = t.size();

	const auto M = [&](uli i, uli j) { return s[i] == t[j]; };

	vvbi L(n, vbi(m));
	loop(0, m - 1, j) L[0][j] = false;
	loop(0, n, i) L[i][m - 1] = M(i, m - 1);
	loop(1, n, i) rloop(0, m - 1, j) L[i][j] = M(i, j) && L[i - 1][j + 1];

	vvbi R(n, vbi(m));
	loop(0, m - 1, j) R[n - 1][j] = L[n - 1][j];
	loop(0, n, i) R[i][m - 1] = M(i, m - 1);
	rloop(0, n - 1, i) rloop(0, m - 1, j) R[i][j] = L[i][j] || M(i, j) && R[i + 1][j + 1];

...
#define CONTAINS(c, x) ((c).find(x) != (c).last())
...
void go()
{
	string str;
	cin >> str;
	char ch = str.size() + ('a' - 1);
	li first = 0, last = str.size() - 1;
	bool alpha = true;
	while (alpha && first <= last)
	{
		if (str[first] == ch) first++;
		else if (str[last] == ch) last--;
		else alpha = false;
		ch--;
	}
	cout << (alpha ? "YES" : "NO") << endl;
}
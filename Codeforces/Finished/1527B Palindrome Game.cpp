...
{
	uli a, b;
	bool r; // reversal allowed
	bool m;
	Entry(uli a, uli b, bool r, bool m) :
		a(a), b(b), r(r), m(m)
	{}
};
...
void go()
{
	constexpr li NIL = MAX(li);
	const uli T = readr<uli>();
	vector<vector<array<array<li, 2>, 2>>> DP(1001, vector<array<array<li, 2>, 2>>(1001, array<array<li, 2>, 2>{ array<li, 2>{NIL, NIL}, array<li, 2>{NIL, NIL} }));
	vector<vector<array<array<bi, 2>, 2>>> V(1001, vector<array<array<bi, 2>, 2>>(1001, array<array<bi, 2>, 2>{array<bi, 2>{false, false}, array<bi, 2>{false, false}}));
	vector<Entry> S;
	S.reserve(1002001);
	loop(0, T, t)
	{
		const uli n = readr<uli>();
		//vector<vector<array<li, 2>>> DP(n + 1, vector<array<li, 2>>(n + 1, array<li, 2>{NIL, NIL}));
		//vector<vector<array<bi, 2>>> V(n + 1, vector<array<bi, 2>>(n + 1, array<bi, 2>{false, false}));
		uli A = 0, B = 0;
		bool M;
		{
			string str;
			...
			M = (n & 1) && str[n / 2] == '0';
			S.emplace_back(A, B, true, M);
		}
		while (!S.empty())
		{
			const Entry e = S.back();
			const uli &a = e.a, &b = e.b;
			const bool &r = e.r, &m = e.m;
			li& dp = DP[a][b][r][m];
			bool& v = V[a][b][r][m];
			if (dp != NIL)
				S.pop_back();
			...
			else if (v)
			{
				...
				li val = MAX(li);
				if (b != 0)
					val = min(val, -DP[a + 1][b - 1][true][m] + 1);
				if (a != 0)
					val = min(val, -DP[a - 1][b][true][m] + 1);
				if (r && a != 0)
					val = min(val, -DP[a][b][false][m]);
				if (m)
					val = min(val, -DP[a][b][true][false] + 1);
				dp = val;
			}
			else
			{
				if (b != 0)
					S.emplace_back(a + 1, b - 1, true, m);
				if (a != 0)
					S.emplace_back(a - 1, b, true, m);
				if (r && a != 0)
					S.emplace_back(a, b, false, m);
				if (m)
					S.emplace_back(a, b, true, false);
				v = true;
			}
		}
		li val = DP[A][B][true][M];
		cout << (val < 0 ? "ALICE" : val == 0 ? "DRAW" : "BOB") << endl;
	}
}
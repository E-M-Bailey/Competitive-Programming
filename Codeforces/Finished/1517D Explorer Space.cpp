...
int main()
{
	...
	vvuli H(n, vuli(m - 1, 0));
	vvuli V(n - 1, vuli(m, 0));
	loop(0, n, i)
	{
		loop(0, m - 1, j)
		{
	...
	vector<vvuli> DP;
	DP.reserve(k + 1);
	DP.emplace_back(n, vuli(m, 0));
	
	loop(0, k, x)
	{
		DP.emplace_back(n, vuli(m, MAX(uli)));
		const vvuli& prev = DP[x];
		vvuli& cur = DP[x + 1];
		loop(0, n, i)
		{
			loop(0, m, j)
			{
				...
				if (i > 0)
				{
					best = min(best, V[i - 1][j] + prev[i - 1][j]);
				}
				if (j > 0)
				{
					best = min(best, H[i][j - 1] + prev[i][j - 1]);
				}
				if (i < n - 1)
				{
					best = min(best, V[i][j] + prev[i + 1][j]);
				}
				if (j < m - 1)
				{
					best = min(best, H[i][j] + prev[i][j + 1]);
				}
				cur[i][j] = best;
			}
		}
	}
	const vvuli& res = DP[k];
	loop(0, n, i)
	{
		loop(0, m, j)
		{

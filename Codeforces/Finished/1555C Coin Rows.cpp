...
void go()
{
	...
	loop(0, m, i) cin >> T[i];
	loop(0, m, i) cin >> B[i];
	// TS[i] sums T[j] for j in [i, m)
	vuli TS(m + 1);
	TS[m] = 0;
	rloop(0, m, i) TS[i] = TS[i + 1] + T[i];
	// BP[i] sums B[j] for j in [0, i)
	vuli BP(m + 1);
	BP[0] = 0;
	loop(0, m, i) BP[i + 1] = BP[i] + B[i];
	
	uli minVal = max(TS[1], BP[0]);
	uli mindex = 0;
	loop(1, m, i)
	{
		uli val = max(TS[i + 1], BP[i]);
		if (val < minVal)
		{
			minVal = val;
			mindex = i;

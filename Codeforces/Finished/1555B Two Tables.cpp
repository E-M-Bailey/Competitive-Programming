...
void go()
{
	uli W, H, x1, y1, x2, y2, w, h;
	cin >> W >> H >> x1 >> y1 >> x2 >> y2 >> w >> h;
	uli sl = x1, sr = W - x2, sb = y1, st = H - y2;
	bool fitsX = sl + sr >= w, fitsY = sb + st >= h;
	if (!fitsX && !fitsY)
	{
		cout << -1 << endl;
		return;
	}
	ldf dx = INF(ldf), dy = INF(ldf);
	if (fitsX)
	{
		uli sx = max(sl, sr);
		if (sx >= w) dx = 0;
	...
	cout << fixed << setprecision(numeric_limits<ldf>::max_digits10) << min(dx, dy) << endl;

...
struct cplx
{
	...
	inline constexpr cplx& operator+=(cplx c);
	inline constexpr cplx& operator-=(cplx c);

	inline constexpr cplx& operator*=(cplx c);
	inline constexpr cplx& operator*=(T x);
};
...
inline constexpr uli bitrev(uli x, uli n)
{
	uli ret = x & 1;
	for (x >>= 1, n >>= 1; x; x >>= 1)
	{
...
template<typename T>
using vcplx = vector<cplx<T>>;
template<typename T>
using vvcplx = vector<vcplx<T>>;

template<typename T>
inline constexpr cplx<T> operator+(cplx<T> l, cplx<T> r)
{
...
inline constexpr cplx<T> operator*(cplx<T> l, cplx<T> r)
{
	return { l.r * r.r - l.i * r.i, l.r * r.i + l.i * r.r };
}

template<typename T>
inline constexpr cplx<T>& cplx<T>::operator+=(cplx c)
{
	r += c.r;
	i += c.i;
	return *this;
...
template<typename T>
inline constexpr cplx<T>& cplx<T>::operator*=(T x)
{
	r *= x;
	i *= x;
	return *this;
...
inline vcplx<T> dft(const vcplx<T>& X)
{
	const uli n = X.size();
	vcplx<T> Y(n);
	for (uli i = 0; i < n; i++)
	...
	{
		T theta = TAO<T> / m;
		cplx<T> w1 = { cos(theta), inverse ? sin(theta) : -sin(theta) };
		const uli halfM = m / 2;
		for (uli i = 0; i < n; i += m)
		{
			cplx<T> wj = { 1, 0 };
			for (uli j = 0; j < halfM; j++)
			{
				cplx<T> t = Y[i + j];
				cplx<T> u = Y[i + j + halfM];
				Y[i + j] = t + u;
				Y[i + j + halfM] = t - u;
				wj *= w1;
			}
...
template<typename T>
inline void roundDiv(vcplx<T>& X)
{
	uli n = X.size();
	loop(0, n, i)
	{
...
/*int main()
{*/
//vulli c = {1, 3, 2, 5, 7, 4, 5, 3};
//vulli c3 = { 1, 1, 1, 1, 1, 1, 1, 2 };
//vulli c = { 1, 2, 3, 4, 0, 0, 0, 0 };
//vulli c3 = { 2, 3, 4, 5, 0, 0, 0, 0 };
//vpldf s = fft(c);
//vpldf s2 = fft(c3);
//vpldf s3(8);
//loop(0, 8, i)
...
//}
//vpldf inv = ifft(s3);
//vulli c2 = roundDiv(ifft(s3));
//cout << "hi";
//}

template<typename T>
inline void shiftAdd(vcplx<T>& coeffs)
{
	rloop(1, coeffs.size(), i)
		coeffs[i] = coeffs[i - 1];
	coeffs[0].r = 1;
	coeffs[0].i = 0;
}
...
int main()
{
	...
	// Read and arbitrarily root the tree
	{
		vvuli graph(n);
		loop(1, n, i)
		{
	...
	const uli npow2 = rupow2(n);
	vvcplx<ldf> prods(n, vcplx<ldf>(npow2));
	vvcplx<ldf> coeffs(n, vcplx<ldf>(npow2));
	// Evaluate the tree
	{
		vuli dfs1, dfs2;
		dfs1.reserve(n);
		...
		while (!dfs2.empty())
		{
			uli v = dfs2.back();
			dfs2.pop_back();
			const vuli& children = tree[v];
			if (children.empty())
			{
				// leaf node
				coeffs[v][0].r = 1;
				prods[v][0].r = 1;
				//samples[n].resize(npow2);
				//loop(0, npow2, i)
					//samples[n][i] = { 1, 0 };
				continue;
			}
			...
			shiftAdd(prods[children[0]]);
			vcplx<ldf> Bs = dft<ldf, false>(prods[children[0]]);
			vcplx<ldf> tempCplx = dft<ldf, true>(Bs);
			//vulli tempInt = roundDiv(tempCplx);
			loop(1, children.size(), i)
			{
				shiftAdd(prods[children[i]]);

				vcplx<ldf> mul = dft<ldf, false>(prods[children[i]]);
				loop(0, npow2, i)
					Bs[i] *= mul[i];
				//tempCplx = ifft(Bs);
				//tempInt = roundDiv(tempCplx);
				vcplx<ldf> tempCplx = dft<ldf, true>(Bs);
			}
			prods[v] = dft<ldf, true>(Bs);
			roundDiv(prods[v]);
			loop(0, npow2, i)
			{
				coeffs[v][i] = A[i] + prods[v][i];
			}
		}
	}

...
inline constexpr bool getBit(pulli mask, uli i)
{
	return i < 64 ? mask.first & 1ull << i : mask.second & 1ull << (i - 64);
}

inline constexpr pulli setBit(pulli mask, uli i)
{
	if (i < 64) mask.first |= 1ull << i;
	else mask.second |= 1ull << (i - 64);
	return mask;
}

inline constexpr pulli operator|(pulli m1, pulli m2)
{
...
inline constexpr bool hasSet(pulli mask)
{
	return mask.first | mask.second;
}

constexpr pulli ZERO{ 0ull, 0ull };

//pulli dfs(uli u, const vvuli& graph, vbi& hb, vbi& pd, pulli dec)
//{
//	const vuli& adj = graph[u];
//	uli n = adj.size();
//	if (pd[u] || n == 0) return setBit(ZERO, u);
//	pd[u] = pd[u] || getBit(dec, u);
//	pulli newDec = setBit(dec, u);
//	if (n == 1)
//	{
//		uli p = adj[0];
//		pulli anc = dfs(p, graph, hb, pd, newDec);
//		hb[u] = hb[u] || hb[p];
//		return setBit(anc, u);
//	}
//	else // n == 2
//	{
//		uli l = adj[0], r = adj[1];
//		pulli lAnc = dfs(l, graph, hb, pd, newDec);
//		pulli rAnc = dfs(r, graph, hb, pd, newDec);
//		hb[u] = hb[u] || hasSet(lAnc & rAnc);
//		return setBit(lAnc | rAnc, u);
//	}
//}

//inline pair<vuli, vuli> comp(const vvuli& graph)
//{
//	uli n = (uli)graph.size();
//	vbi hb(n, false), pd(n, false);
//	vector<pulli> dec(n, ZERO);
//	bool 
//	pair<vuli, vuli> ret;
//
//	dfs(root, graph, hb, pd, ZERO);
//	loop(0, n, u)
//	{
//		if (pd[u]) ret.second.push_back(u);
//		else if (hb[u]) ret.first.push_back(u);
//	}
//	return ret;
//}
...
{
	uli n = graph.size();
	vbi visitedEver(n, false);
	vbi pd(n, false), hb(n, false);
	loop(0, n, i)
	{
		if (visitedEver[i]) continue;
		vbi visited(n, false);
		vbi inCurPath(n, false);
		vuli prev(n);
		vuli st;
		vuli curPath;
		st.reserve(n);
		...
		while (!st.empty())
		{
			...
			if (!inCurPath[u]) // going down
			{
				...
				for (uli v : adj)
				{
					...
					{
						pd[v] = true;
						for (uli j = curPath.size(); j-- > 0 && curPath[j] != v;)
							pd[j] = true;
						continue;
					}
					if (visited[v])
			...
			else // going back up
			{
				...
				for (uli v : adj)
				{
					pulli vAnc = setBit(ancs[v], v);
					if (hasSet(vAnc & anc)) hb[u] = true;
					anc = anc | vAnc;
				}
				if (!hb[u])
					for (uli v : adj)
	...
	for (uli u = 0; u < n; u++)
	{
		if (pd[u]) res.second.push_back(u);
		else if (hb[u]) res.first.push_back(u);
	}
	return res;
}
...
void go()
{
	...
	auto getIdx = [&](string& name) -> uli
	{
		unordered_map<string, uli>::iterator iter = nameToIdx.find(name);
		if (iter == nameToIdx.end())
		{
			uli ret = (uli)p2c.size();
			nameToIdx.emplace(name, ret);
			idxToName.emplace_back(move(name));
			p2c.push_back(vuli());
			c2p.push_back(vuli());
			return ret;
		}
	...
	while (getline(cin, line) && !line.empty())
	{
		if (line == "done")
		{
			pair<vuli, vuli> res = comp(c2p);
			map<string, bool> output;
			for (uli i : res.first) output.emplace(move(idxToName[i]), false);
			for (uli i : res.second) output.emplace(move(idxToName[i]), true);
			for (const pair<string, bool>& line : output)
				cout << line.first << (line.second ? " paradox\n" : " hillbilly\n");
			cout << endl;
			idxToName.clear();
			nameToIdx.clear();
			p2c.clear();
		...
		else
		{
			...
			string parent, begat, child;
			strm >> parent >> begat >> child;
			uli p = getIdx(parent), c = getIdx(child);
			p2c[p].push_back(c);
			c2p[c].push_back(p);
		}
	}

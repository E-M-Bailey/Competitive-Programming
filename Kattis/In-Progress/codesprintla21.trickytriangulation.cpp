...
struct vec2
{
	...
	template<typename U>
	inline constexpr explicit operator vec2<U>()
	{
...
template<typename T>
inline istream& operator>>(istream& strm, vec2<T>& v)
{
	return strm >> v.x >> v.y;
}

template<typename T>
inline ostream& operator<<(ostream& strm, const vec2<T>& v)
{
	return strm << v.x << ' ' << v.y;
}
...
template<typename T>
inline vec2<T> operator+(vec2<T> l, vec2<T> r)
{
...
template<typename T>
inline bool operator==(vec2<T> l, vec2<T> r)
{
	return l.x == r.x && r.y == r.y;
}

template<typename T>
inline T operator*(vec2<T> l, vec2<T> r)
{
...
template<typename T>
inline vec2<T> operator*(vec2<T> l, T r)
{
	return { l.x * r, l.y * r };
}

template<typename T>
inline vec2<T> operator*(T l, vec2<T> r)
{
	return { l * r.x, l * r.y };
}
...
template<typename T>
inline vec2<T> floor(vec2<T> v)
{
	return { floor(v.x), floor(v.y) };
}

typedef ldf usedtype;
typedef vec2<usedtype> v2lli;
typedef vec2<ldf> v2ldf;

void go()
{
	v2lli a1, a2, b1, b2;
	cin >> a1 >> a2 >> b1 >> b2;
	v2lli da = a2 - a1, db = b2 - b1;
	...
	v2lli c = { a1 ^ a2, b1 ^ b2 };
	v2lli pd = { c ^ v2lli{da.x, db.x}, c ^ v2lli{da.y, db.y} };

	v2lli a1d = a1 * d, a2d = a2 * d, b1d = b1 * d, b2d = b2 * d;
	v2lli af = pd - a1d, bf = pd - b1d;
	usedtype A = af * da, B = bf * db;

	usedtype A1 = d > 0 ? a1d * da : a2d * da, A2 = d > 0 ? a2d * da : a1d * da, Ap = pd * da;
	usedtype B1 = d > 0 ? b1d * db : b2d * db, B2 = d > 0 ? b2d * db : b1d * db, Bp = pd * db;

	//if (A1 > Ap || Ap > A2 || B1 > Bp || Bp > B2)
	if (A1 > Ap || B1 > Bp)
		cout << -1;
	else
	{
		constexpr usedtype BIG = 1000000000;
		v2lli res = (v2lli)floor((v2ldf)pd / (ldf)d);
		if (res.x < -BIG || res.x > BIG || res.y < -BIG || res.y > BIG)
			cout << -1;
		else
			cout << res;
	}

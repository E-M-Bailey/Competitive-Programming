...
// comparator based on first
inline constexpr bool operator<(const puli& p1, const puli& p2)
{
	return p1.first < p2.first;
}

inline void multi_push_back(vpuli& vec, uli val)
{
	if (vec.empty() || vec.back().first != val)
	{
...
//	Node* next;
//
//	inline constexpr Node(ulli val) :
//		val(val), prev(this), next(this)
//	{}
//
//	inline constexpr Node(ulli val, Node* prev, Node* next) :
//		val(val), prev(prev), next(next)
//	{}
//
//	inline constexpr void push_front(ulli val)
//	{
//		Node* n = new Node(this->val, this, next);
//		n
//	}
//
//	inline constexpr void push_back(ulli val)
//	{
//		Node* n = new Node{ val, prev, this };
//		prev->next = n;
//		prev = n;
//	}
//};

inline constexpr ulli merge(list<uli>& a, list<uli>& b)
{
...
int main()
{
	...
	loop(0, n, i)
	{
		...
		sections.back().push_back(h);
		prev = h;
	}

	list<uli>& accum = sections[0];
	loop(1, sections.size(), i)
	{
		merge(accum, sections[i]);
	}

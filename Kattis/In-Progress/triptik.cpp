...
template<typename T>
inline constexpr T gcd(T lc, T rc)
{
	if (!(lc && rc))
	{
		return lc | rc;
	}
	int s = __builtin_ctz(lc | rc);
	lc >>= __builtin_ctz(lc);
	do
	{
		rc >>= __builtin_ctz(rc);
...
template<typename T>
inline constexpr T lcm(T lc, T rc) // l and r should be nonzero
{
	return lc / gcd(lc, rc) * rc;
}
...
constexpr uli NIL = MAX(uli);
uli* X;

void merge(array<uli, 4>& dest, const array<uli, 4>& A, const array<uli, 4>& B)
{
	uli i = 0, a = 0, b = 0;
	while (i < 4)
	{
		if (A[a] != NIL && A[a] != 0 && A[a] > B[b])
			dest[i++] = A[a++];
		else
			dest[i++] = B[b++];
	}
...
struct Node
{
	...
	array<uli, 4> v;
	Node *lc, *rc;
	Node(uli i) : ct(1), v{ i, NIL, NIL, NIL }, lc(nullptr), rc(nullptr) {}
	void add(uli i, uli l, uli r)
	{
		uli m = (l + r) / 2;
		if (ct == 1)
			(X[v[0]] <= m ? lc : rc) = new Node(v[0]);
		if (ct < 4) ct++;
		v[3] = v[2];
		v[2] = v[1];
		v[1] = v[0];
		...
		if (ct > 1)
		{
			...
			Node*& n = goL ? lc : rc;
			if (!n) n = new Node(i);
			else if (goL) n->add(i, l, m);
			else n->add(i, m + 1, r);
		}
	}
	void get(array<uli, 4>& vals, uli l, uli r, uli nl, uli nr)
	{
		if (l == nl && r == nr)
		{
			array<uli, 4> temp = vals;
			merge(vals, temp, v);
		}
		else
		{
			uli m = (nl + nr) / 2;
			if (l <= m) get(vals, l, min(m, r), nl, m);
			if (r > m) get(vals, max(m + 1, l), r, m + 1, nr);
		}
	}
};
...
struct BiTree
{
	...
	void add(uli i)
	{
		root->add(i, 0, 200000000);
	}
	void get(array<uli, 4>& v, uli l, uli r)
	{
		root->get(v, l, r, 0, 200000000);
	}
};
...
void go()
{
	...
	BiTree tree;
	loop(2, n, i) tree.add(i);

	vvuli graph(n * 30);
	loop(0, n, i) loop(0, 30, z)
	{
		uli x = X[i];
		uli d = z ? 1u << z - 1 : 0;
		uli l = x - min(d, x), r = x + min(d, 200000000 - x);
		array<uli, 4> v{ NIL, NIL, NIL, NIL };
		tree.get(v, l, r);
		loop(0, 4, j)
		{
			uli val = v[j];
			if (val != NIL && val != 0)
			{
				graph[i * 30 + z].push_back(val * 30 + z);
			}
		}
		if (z > 0) graph[i * 30 + z].push_back(i * 30 + z - 1);
		if (z < 29) graph[i * 30 + z].push_back(i * 30 + z + 1);
	}
	...
	while (!Q.empty())
	{
		uli v = Q.front(), d = dist[v];
		Q.pop();
		
		for (uli w : graph[v])
		{
			if (dist[w] != NIL) continue;
			dist[w] = d + 1;
			Q.push(w);
		}
	}

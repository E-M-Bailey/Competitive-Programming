...
struct SegTree
{
	...
	inline void build(const vector<T>& vec, uli v, uli tl, uli tr)
	{
		if (tl == tr)
			data[v] = vec[tl];
		else
	...
	inline SegTree(uli n, Op op, T id) :
		SegTree(vector<T>(n, id), op, id)
	{}

	inline T sum(uli v, uli tl, uli tr, uli l, uli r)
	{
		if (l > r) return id;
		if (l == tl && r == tr) return data[v];
		uli tm = (tl + tr) / 2;
		uli lBound = min(r, tm);
		uli rBound = max(l, tm + 1);
		T lSum = sum(v * 2, tl, tm, l, lBound);
		T rSum = sum(v * 2 + 1, tm + 1, tr, rBound, r);
		return op(lSum, rSum);
	}

	inline T sum(uli l, uli r)
	{
		return sum(1, 0, n - 1, l, r);
	}

	inline void update(uli v, uli tl, uli tr, uli pos, T newVal)
	{
		if (tl == tr)
			data[v] = newVal;
		else
	...
	inline void update(uli pos, T newVal)
	{
...
{
	li M1, M2, m1, m2;
	Data1d(li M1, li M2, li m1, li m2) :
		M1(M1), M2(M2), m1(m1), m2(m2)
	{}
	Data1d() :
		Data1d(MIN(li), MIN(li), MAX(li), MAX(li))
	{}
	Data1d(li val) :
		Data1d(val, val, val, val)
	{}
};

Data1d combine(const Data1d& l, const Data1d& r)
{
	li values[8]{ l.M1, l.M2, l.m1, l.m2, r.M1, r.M2, r.m1, r.m2 };
	sort(values, values + 8);
	return Data1d{ values[7], values[6], values[0], values[1] };
}

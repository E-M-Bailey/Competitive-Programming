...
void go()
{
	map<string, uli> nameToIdx;
	vector<string> idxToName;
	vvuli graph;

	const auto addName = [&](string& str) -> uli
	{
		map<string, uli>::iterator itr = nameToIdx.find(str);
		if (itr != nameToIdx.end()) return itr->second;
		uli ret = idxToName.size();
		graph.push_back(vuli());
		nameToIdx.emplace(str, ret);
		idxToName.emplace_back(move(str));
		return ret;
	};
	...
	loop(0, n, i)
	{
		...
		loop(0, n, j)
		{
			uli xCt, yCt;
			string strX, strY;
			cin >> xCt, strX, yCt, strY;
			uli x = addName(strX), y = addName(strY);
			x
		}
	}
}
...
ldf compH(ldf V, ldf r)
{
	ldf r2 = r * r;
	ldf r3 = r2 * r;
	ldf a = PI2 * r3 - 3 * V;
	ldf b = sqrtl(3 * V * (3 * V - PI4 * r3));
	ldf c = cbrtl((a + b) / PI2);
	return c + r + r2 / c;
}

ldf solveCubic(ldf a, ldf b, ldf c, ldf d)
{
	if (a == 0)
		if (b == 0)
			if (c == 0)
			...
			return (-c + sqrt(c * c - 4 * b * d)) / (2 * b);
	ldf d0 = b * b - 3 * a * c;
	ldf d1 = 2 * b * b * b - 9 * a * b * c + 27 * a * a * d;
	if (d0 == 0 && d1 == 0) return -b / (3 * a);
	ldf d = sqrtl(d1 * d1 - 4 * d0 * d0 * d0);
	ldf c = d1;
	if (d1 < 0) c -= d;
	else c += d;
	ldf C = cbrtl(c / 2);
...
void go()
{
	...
	loop(0, n, i)
	{
		...
		//ldf c = cbrtl((a + b) / PI2);
		//ldf ph = c + r + r2 / c;
		PH[i] = solveCubic(-PI / 3, PI * r, 0, 0);
	}
	const auto pred = [&PH](uli l, uli r) -> bool
	{
		return PH[l] < PH[r] || PH[l] == PH[r] && l < r;
	};
	vuli perm(n);
	iota(ALL(perm), 0);
	sort(ALL(perm), pred);

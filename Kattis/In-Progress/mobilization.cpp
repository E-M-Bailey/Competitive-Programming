...
void go()
{
	...
	loop(0, n, i)
	{
		...
		{
			ldf val, x;
			bool hSame = H[i] * C[j] == H[j] * C[i];
			bool pSame = P[i] * C[j] == P[j] * C[i];
			ldf a = H[i] / C[i], b = H[j] / C[j], c = P[i] / C[i], d = P[j] / C[j];
			if (hSame && pSame)
			{
				val = a * c * B * B;
			}
			...
			{
				x = (b * (2 * d - c) - a * d) / (2 * (a - b) * (c - d));
				x = clamp(x, 0.l, 1.l);
				val = (x * a + (1 - x) * b) * (x * c + (1 - x) * d) * B * B;
			}
			//ldf val, x;
			//ldf h1 = H[i], h2 = H[j], p1 = P[i], p2 = P[j];
			//if (h1)
			//if (i == j) val = b * b * H[i] * P[i];
			//else
			//{
			//	x = b * 0.5l * (H[j] / (H[j] - H[i]) + P[j] / (P[j] - P[i]));

			//	val = (x * H[i] + (b - x) * H[j]) * (x * P[i] + (b - x) * P[j]);
			//}
			if (val > maxVal)
				maxVal = val;
		}
	}
	cout << fixed << setprecision(3) << maxVal;
}
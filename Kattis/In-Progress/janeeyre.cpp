...
vector<pair<string, uli>> books; // (title, pages)
vector<puli> gifts; // (idx in books, time)

inline bool titleGreater(uli a, uli b)
{
	return books[a].first > books[b].first;
}

inline bool timeGreater(uli a, uli b)
{
	return gifts[a].second > gifts[b].second;
}
...
int main()
{
	...
	uli n, m, k;
	cin >> n >> m >> k;
	priority_queue<uli, vector<uli>, decltype(&titleGreater)> received(titleGreater); // indices in books
	priority_queue<uli, vector<uli>, decltype(&timeGreater)> unreceived(timeGreater); // indices in gifts

	books.emplace_back("Jane Eyre", k);
	received.push(0);
	...
	loop(0, n, i)
	{
		string si;
		uli ki;
		getline(cin.ignore(MAX(streamsize), '"'), si, '"') >> ki;
		books.emplace_back(si, ki);
		received.push(books.size() - 1);
	}
	...
	loop(0, m, j)
	{
		...
		string sj;
		uli kj;
		getline((cin >> tj).ignore(MAX(streamsize), '"'), sj, '"') >> kj;
		books.emplace_back(sj, kj);
		gifts.emplace_back(books.size() - 1, tj);
		unreceived.push(gifts.size() - 1);
	}

	ulli time = 0;
	while (true)
	{
		while (!unreceived.empty() && gifts[unreceived.top()].second <= time)
		{
			received.push(gifts[unreceived.top()].first);
			unreceived.pop();
		}
		time += books[received.top()].second;
		if (books[received.top()].first == "Jane Eyre")
		{
			cout << time;
			break;
		}

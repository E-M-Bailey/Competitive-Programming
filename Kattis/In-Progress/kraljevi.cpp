...
ulli compSpread(const vvbi& grid)
{
	vvuli num(R, vuli(C));
	num[0][0] = grid[0][0];
	loop(1, R, i) num[i][0] = num[i - 1][0] + grid[i][0];
	loop(1, C, j) num[0][j] = num[0][j - 1] + grid[0][j];
	loop(1, R, i) loop(1, C, j) num[i][j] = num[i][j - 1] + num[i - 1][j] - num[i - 1][j - 1] + grid[i][j];
	vvulli dist(R, vulli(C));
	dist[0][0] = 0;
	loop(1, R, i) dist[i][0] = dist[i - 1][0] + num[i - 1][0];
	loop(1, C, j) dist[0][j] = dist[0][j - 1] + num[0][j - 1];
	loop(1, R, i) loop(1, C, j) dist[i][j] = dist[i - 1][j] + num[i - 1][j] + dist[i][j - 1] + num[i][j - 1] - dist[i - 1][j - 1] - 2ull * num[i - 1][j - 1];
	loop(1, R, i) loop(1, C, j) dist[i][j] -= dist[i - 1][j - 1];
	ulli res = 0;
	loop(0, R, i) loop(0, C, j) res += grid[i][j] * dist[i][j];
	return res;
}
...
{
	cin >> R >> C;
	vvbi mGrid(R, vbi(C, false)), sGrid(R, vbi(C, false));
	...
	cout << compSpread(mGrid) << ' ' << compSpread(sGrid);
}
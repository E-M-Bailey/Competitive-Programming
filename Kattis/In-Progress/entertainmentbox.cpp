...
void go()
{
	const auto pred = [](puli l, puli r) -> bool
	{
	...
	cin >> n >> k;
	vpuli A(n);
	loop(0, n, i) cin >> A[i].first >> A[i].second;
	sort(ALL(A), pred);
	priority_queue<uli, vuli, greater<uli>> Q;
	loop(0, k, i) Q.emplace(A[i].second);
	uli idx = k;
	uli res = k;
	while (true)
	{
		uli end = Q.top();
		Q.pop();
		while (idx < n && A[idx].first < end)
			idx++;
		if (idx >= n)
			break;
		Q.emplace(A[idx++].second);
		res++;
	}
	cout << res;
}
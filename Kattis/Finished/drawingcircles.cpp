...
struct Circle
{
	...
	inline Circle() {}
	inline constexpr Circle(li x, li y, li r) : x(x), y(y), r(r) {}

	friend inline istream& operator>>(istream& strm, Circle& circle)
	{
		return strm >> circle.x >> circle.y >> circle.r;
	}
};

const ldf PI = 2 * acosl(0), PI2 = 4 * acosl(0), PI4 = 8 * acosl(0);

//#define ERR_BEG try {
//#define ERR_END } catch(const exception& e) { cout << "qijqiejfoij" << endl; exit(0); }
...
void go()
{
	...
	// first in [0, 2pi), second in [first, first + 2pi)
	vvpldf intervals(N);
	loop(0, N, i)
	{
		Circle& ci = circles[i];
		...
		loop(0, i, j)
		{
			Circle& cj = circles[j];
			if (cj.r == 0) continue;
			lli dx = ci.x - cj.x, dy = ci.y - cj.y, sr = ci.r + cj.r;
			lli dx2 = dx * dx, dy2 = dy * dy, sr2 = sr * sr;
			lli d2 = dx2 + dy2;
			if (d2 >= sr2) continue;
			lli dr = ci.r - cj.r;
			lli dr2 = dr * dr;
			...
			//{
				...
				//exit(0);
			//}
			if (d2 + cj.r * cj.r <= ci.r * ci.r)
				da = PI - da;
			ldf ca = atan2l(dy, dx) + PI4;
			ldf lo = ca - da, hi = ca + da;
			ldf loAdj = fmodl(lo, PI2);
			I.emplace_back(loAdj, hi - lo + loAdj);
		}
	}

	const auto cmp = [](const pldf& l, const pldf& r) -> bool
	{
	...
	loop(0, N, i)
	{
		...
		loop(0, s, i)
		{
			const pldf& iv = I[i];
			//if (iv.second > PI2)
			I.emplace_back(iv.first - PI2, iv.second - PI2);
		}
		sort(ALL(I), cmp);
		ldf curRes = PI2;
		pldf running = I[0];
		...
		{
			const pldf& cur = I[j];
			if (cur.first <= running.second)
				running.second = max(running.second, cur.second);
			else
			{
				curRes -= max(0.0l, running.second) - max(0.0l, running.first);
				running = cur;
			}
		}
		curRes -= clamp(running.second, 0.0l, PI2) - clamp(running.first, 0.0l, PI2);// max(0.0l, running.second) - max(0.0l, running.first);
		res += curRes * r;
	}

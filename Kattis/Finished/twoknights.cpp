...
uli TYPE_LETTER = 0, TYPE_SPACE = 1, TYPE_SHIFT = 2;

array<uli, 40> keyTypes()
{
	array<uli, 40> ret;
	loop(0, 30, i)
		ret[i] = TYPE_LETTER;
	ret[32] = ret[33] = ret[34] = ret[35] = ret[36] = ret[37] = TYPE_SPACE;
	ret[30] = ret[31] = ret[38] = ret[39] = TYPE_SHIFT;
	return ret;
}

unordered_map<char, uli> indices()
{
	unordered_map<char, uli> ret;
	ret['Q'] = ret['q'] = 0;
	ret['W'] = ret['w'] = 1;
	ret['E'] = ret['e'] = 2;
	ret['R'] = ret['r'] = 3;
...
unordered_map<char, bool> isCap()
{
	unordered_map<char, bool> ret;
	ret['Q'] = ret['W'] = ret['E'] = ret['R'] = ret['T'] = ret['Y'] = ret['U'] = ret['I'] = ret['O'] = ret['P'] = ret['A'] = ret['S'] = ret['D'] = ret['F'] = ret['G'] = ret['H'] = ret['J'] = ret['K'] = ret['L'] = ret[':'] = ret['Z'] = ret['X'] = ret['C'] = ret['V'] = ret['B'] = ret['N'] = ret['M'] = ret['<'] = ret['>'] = ret['?'] = true;
	ret['q'] = ret['w'] = ret['e'] = ret['r'] = ret['t'] = ret['y'] = ret['u'] = ret['i'] = ret['o'] = ret['p'] = ret['a'] = ret['s'] = ret['d'] = ret['f'] = ret['g'] = ret['h'] = ret['j'] = ret['k'] = ret['l'] = ret[';'] = ret['z'] = ret['x'] = ret['c'] = ret['v'] = ret['b'] = ret['n'] = ret['m'] = ret[','] = ret['.'] = ret['/'] = false;
	return ret;
}
...
vvuli knightMoves()
{
	...
	vvuli ret(40);
	for (li i = 0; i < 4; i++) for (li j = 0; j < 10; j++)
	{
		uli idx = i * 10 + j;
		for (const pli& move : moves)
		{
			li mi = i + move.first, mj = j + move.second;
			if (mi >= 0 && mj >= 0 && mi < 4 && mj < 10)
				ret[idx].push_back(mi * 10 + mj);
		}
	}
	return ret;
}

pair<vci, vci> lowerUpper()
{
	pair<vci, vci> ret(vci(40), vci(40));
	const unordered_map<char, uli> idcs = indices();
	const unordered_map<char, bool> iscp = isCap();
	for (const pair<char, uli>& p : idcs)
		(iscp.at(p.first) ? ret.second : ret.first)[p.second] = p.first;
	return ret;
}

const array<uli, 40> KEY_TYPES = keyTypes();
const unordered_map<char, uli> INDICES = indices();
const unordered_map<char, bool> IS_CAP = isCap();
const pair<vci, vci> LOWER_UPPER = lowerUpper();
const vvuli KNIGHT_MOVES = knightMoves();
...
struct State
{
	...
	inline State() {}
	inline constexpr State(uli code) : code(code) {}
	inline constexpr State(uli a, uli b, uli i) : code(a + b * 40 + i * 1600) {}
	//inline constexpr State(uli a, uli b, uli i) : code(min(a, b) + max(a, b) * 40 + i * 1600) {}

	inline constexpr uli a() { return code % 40; }
	inline constexpr uli b() { return code % 1600 / 40; }
	inline constexpr uli i() { return code / 1600; }
	inline constexpr bool success() { return code == MAX(uli); }

	friend inline constexpr bool operator==(State l, State r) { return l.code == r.code; }
	friend inline constexpr bool operator!=(State l, State r) { return l.code != r.code; }
};

constexpr State INIT_STATE(30, 39, 0);

template<>
struct std::hash<State>
{
	inline size_t operator()(State state) const noexcept
	{
		return hash<uli>()(state.code);
	}
};

void go(const string& str)
{
	unordered_set<State> states, newStates;
	states.insert(INIT_STATE);
	vbi visited(1600 * str.size(), false);
	visited[INIT_STATE.code] = true;
	bool found = false;
	while (!states.empty())
	{
		newStates.clear();
		...
		{
			uli a = state.a(), b = state.b(), i = state.i();
			uli aType = KEY_TYPES[a], bType = KEY_TYPES[b];
			for (uli na : KNIGHT_MOVES[a])
			{
				if (na == b) continue;
				uli naType = KEY_TYPES[na];
				uli ni = i + (naType != TYPE_SHIFT);
				if (naType == TYPE_SPACE && str[i] != ' ') continue;
				if (naType == TYPE_LETTER && str[i] != (bType == TYPE_SHIFT ? LOWER_UPPER.second : LOWER_UPPER.first)[na]) continue;
				State nState(na, b, ni);
				if (ni == str.size())
				{
					found = true;
					break;
				}
				newStates.insert(nState);
			}
			if (found) break;
			for (uli nb : KNIGHT_MOVES[b])
			{
				if (nb == a) continue;
				uli nbType = KEY_TYPES[nb];
				uli ni = i + (nbType != TYPE_SHIFT);
				if (nbType == TYPE_SPACE && str[i] != ' ') continue;
				if (nbType == TYPE_LETTER && str[i] != (aType == TYPE_SHIFT ? LOWER_UPPER.second : LOWER_UPPER.first)[nb]) continue;
				State nState(nb, a, ni);
				if (ni == str.size())
				{
					found = true;
					break;

...
#define ALL(minC) (minC).begin(), (minC).end()
#define CALL(minC) (minC).cbegin(), (minC).cend()
#define RALL(minC) (minC).rbegin(), (minC).rend()
#define CRALL(minC) (minC).crbegin(), (minC).crend()

#define CONTAINS(minC, x) ((minC).find(x) != (minC).end())
...
void go()
{
	...
	{
		uli c = R[p][0];
		if (CV[c] == 0) rem.push_back(c);
		CV[c] += V[p];
		CP[c].push_back(p);
	}

	const auto pred = [&CV](uli l, uli r)
	{
		uli cvl = CV[l], cvr = CV[r];
		return cvl < cvr || cvl == cvr && l < r;
	};

	while (rem.size() > 1)
	{
		vuli::iterator maxIter = max_element(ALL(rem), pred);
		uli maxC = *maxIter;
		if (CV[maxC] * 2 > totalV)
		{
			rem.front() = maxC;
		...
		vuli::iterator minIter = min_element(ALL(rem), pred);
		uli minC = *minIter;
		for (uli p : CP[minC])
		{
			const vuli& rp = R[p];

...
struct frac
{
	...
	inline constexpr ulli min(ulli a, ulli b, ulli c)
	{
		return min(min(a, b), c);
	}

	inline constexpr ulli max(ulli a, ulli b, ulli c, ulli d)
	{
	...
	struct player
	{
		...
		inline constexpr ulli score()
		{
			return a * a + b * b + c * c + 7 * min(a, b, c);
		}

		inline constexpr ulli score0()
		{
			c += d;
			ulli ret = score();
			c -= d;
		...
		inline constexpr ulli score2()
		{
			ulli toA = min(d, b - a);
			d -= toA;
			a += toA;
			ulli toAB = min(d / 2, c - b);
			d -= 2 * toAB;
			a += toAB;
			b += toAB;
			c += d;
	...
	int main()
	{
		...
		loop(0, n, i)
		{
			...
			cin >> p.a >> p.b >> p.c >> p.d;
			p.sortABC();
			cout << max(p.score0(), p.score1(), p.score2(), p.score3()) << endl;
		}
		return 0;
	}
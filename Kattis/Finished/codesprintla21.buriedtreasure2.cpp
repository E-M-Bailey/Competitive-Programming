...
struct DisjointSet
{
		...
		nodes(n)
	{
		...
		{
			pli p = readr<pli>();
			nodes[i] = Node{ nullptr, 0, p.first, p.second };
		}
...
bool scc(uli v, uli m, const vvuli& graph, vuli& S, vuli& number, vuli& lowpt, vbi& onStack, uli& i)
{
	number[v] = i;
	lowpt[v] = i;
	i++;
	...
	for (uli w : graph[v])
	{
		if (number[w] == NIL)
		{
			if (!scc(w, m, graph, S, number, lowpt, onStack, i)) return false;
	...
	if (number[v] == lowpt[v])
	{
		...
		do
		{
			...
			onStack[w] = false;
			uli negW = 2 * m - 1 - w;
			if (curSCC.find(negW) != curSCC.end()) return false;
			curSCC.insert(w);
		}
		while (w != v);
	}
...
void go()
{
	...
	const auto addEdge = [&](li a, li b)
	{
		uli i = toIdx(a), j = toIdx(b);
		graph[i].push_back(j);
	};
	...
	uli idx = 0;
	vuli number(size, NIL), lowpt(size, NIL);
	vbi onStack(size, false);
	vuli S;
	S.reserve(size);
	loop(0, n, i)
	{
		if (number[i] == NIL)
			if (!scc(i, m, graph, S, number, lowpt, onStack, idx))
			{
				cout << "NO";
				return;
			}

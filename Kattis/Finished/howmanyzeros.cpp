...
inline constexpr ulli G(ulli n, uli p, ulli m);

inline constexpr ulli H(ulli n, uli p, ulli m)
{
	ulli ret = n ? min(m, n) + G(n, p, m) : 0;
	return ret;
}
...
inline constexpr ulli G(ulli n, uli p, ulli m)
{
	if (!p || !n) return 0;
	ulli r = n / m, ret = 0;
	if (!r) ret = G(n % m, p - 1, m / 10);
	else ret = G(m, p - 1, m / 10) + (r - 1) * H(m, p - 1, m / 10) + H(n % m, p - 1, m / 10);
	return ret;
}

inline constexpr ulli F(ulli n)
{
	if (n == 0) return 0;
	ulli m = 1;
	ulli res = 1;

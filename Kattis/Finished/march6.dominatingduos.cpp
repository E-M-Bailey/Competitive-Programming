...
int main()
{
	...
	cin >> d;
	d--;
	cTree.push_back({ d, MAX(uli), MAX(uli), MAX(uli), MAX(uli), MAX(uli), MAX(uli) });
	...
	while (!dfs.empty())
	{
		uli i = dfs.back();
		node& nd = cTree[i];
		bool hasL = nd.l != MAX(uli), hasR = nd.r != MAX(uli);
		if (hasL ? cTree[nd.l].a == MAX(uli) : hasR && cTree[nd.r].a == MAX(uli))
		{
			if (hasL)
				dfs.push_back(nd.l);
			if (hasR)
		...
		else
		{
			...
			if (hasL)
			{
				const node& l = cTree[nd.l];
				nd.a = l.a + 1;
				nd.b = l.c + 1;
			}
			if (hasR)
			{
				const node& r = cTree[nd.r];
				nd.b += r.a + 1;
				nd.c = r.c + 1;
			}
			sum += nd.b;

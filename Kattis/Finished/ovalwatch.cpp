...
{
	ui N, K; cin >> N >> K;
	vui V(N); loop(0, N, i) V[i] = i;
	vpui BA(K); loop(0, K, i) cin >> BA[i].second >> BA[i].first;
	sort(BA.begin(), BA.end(), greater<pui>());
	for (pui ba : BA) swap(V[ba.second], V[ba.second + 1]);
	cout << V[0]; loop(1, N, i) cout << ' ' << V[i];
	return 0;
}
...
inline pldf mkV(long double angle)
{
...
int main()
{
	...
	constexpr const ldf inf = numeric_limits<ldf>::infinity();
	const ldf PI = atanl(1.0l) * 4;

	ldf maxD[]{ 0, 0, 0, 0, 0, 0 };
	ldf minD[]{ inf, inf, inf, inf, inf, inf };
	...
	pldf *dirs[]
	{
		new pldf[3]{ mkV(PI / 3), mkV(PI), mkV(5 * PI / 3) },
		new pldf[4]{ mkV(PI / 4), mkV(3 * PI / 4), mkV(5 * PI / 4), mkV(7 * PI / 4) },
		new pldf[5]{ mkV(PI / 5), mkV(3 * PI / 5), mkV(PI), mkV(7 * PI / 5), mkV(9 * PI / 5) },
		new pldf[6]{ mkV(PI / 6), mkV(PI / 2), mkV(5 * PI / 6), mkV(7 * PI / 6), mkV(3 * PI / 2), mkV(11 * PI / 6) },
		new pldf[7]{ mkV(PI / 7), mkV(3 * PI / 7), mkV(5 * PI / 7), mkV(PI), mkV(9 * PI / 7), mkV(11 * PI / 7), mkV(13 * PI / 7) },
		new pldf[8]{ mkV(PI / 8), mkV(3 * PI / 8), mkV(5 * PI / 8), mkV(7 * PI / 8), mkV(9 * PI / 8), mkV(11 * PI / 8), mkV(13 * PI / 8), mkV(15 * PI / 8) }
	};
	...
	loop(0, n, i)
	{
		...
		loop(3, 9, i)
		{
			...
			loop(0, i, j)
			{
				const pldf& norm = dir[j];
				ldf d = pt.first * norm.first + pt.second * norm.second;
				if (d > dist)
				{
					dist = d;
				}
	...
	cout << minKIdx << ' ' << setprecision(numeric_limits<ldf>::digits10) << 1 / (minKSqrt * minKSqrt);

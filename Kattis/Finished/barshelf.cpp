...
int main()
{
	...
	loop(0, n, i)
	{
		compressed[sorted[i].second] = i;
	}

	vuli maxLHalf(n), minGDouble(n);

	{
		uli j = n;
		rloop(0, n, i)
		{
			uli iVal = sorted[i].first;
			while (j > 0 && 2 * sorted[j - 1].first > iVal)
			{
				j--;
			}
			maxLHalf[sorted[i].second] = j == 0 ? n : j - 1;
		}
		...
		{
			uli iVal = sorted[i].first;
			while (j < n && sorted[j].first < 2 * iVal)
			{
				j++;
			}
			minGDouble[sorted[i].second] = j;
		}
	}

	vuli smaller(n), greater(n);
	BIT bit(n);

	rloop(0, n, i)
	{
		smaller[i] = maxLHalf[i] == n ? 0 : bit.sum(maxLHalf[i]);
		bit.increment(compressed[i]);
	}
	...
	loop(0, n, i)
	{
		greater[i] = minGDouble[i] == n ? 0 : i - bit.sum(minGDouble[i] - 1);
		bit.increment(compressed[i]);
	}
	...
	loop(0, n, i)
	{
		res += (ulli)smaller[i] * (ulli)greater[i];
	}

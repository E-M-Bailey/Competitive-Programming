...
inline constexpr bool operator<(const pli& l, const pli& r)
{
...
struct std::hash<pli>
{
	inline constexpr size_t operator()(const pli& x) const noexcept
	{
		return (static_cast<size_t>(x.first) << 32) | x.second;
	}
};
...
{
	vpli P(n);
	loop(0, n, i) cin >> P[i].first >> P[i].second;
	if (n <= 2)
	{
		cout << n << endl;
		return;
	...
	loop(0, n, i)
	{
		vpli D;
		D.reserve(n - 1);
		unordered_map<pli, uli> M;
		loop(0, n, j)
		{
			if (j == i) continue;
			pli d(P[j].first - P[i].first, P[j].second - P[i].second);
			if (d.first < 0 || d.first == 0 && d.second < 0)
			{
				d.first = -d.first;
				d.second = -d.second;
			}
			li g = gcd(abs(d.first), abs(d.second));
			d.first /= g;
			d.second /= g;
			D.push_back(d);
			//M[d]++;
		}
		for (const puli& d : D) maxCol = max(++M[d] + 1, maxCol);
	}
	cout << maxCol << endl;
	//loop(1, n, j) loop(0, j, i)
	//{
	//	pli d(P[j].first - P[i].first, P[j].second - P[i].second);
	//	if (d.first < 0 || d.first == 0 && d.second < 0)
	//	{

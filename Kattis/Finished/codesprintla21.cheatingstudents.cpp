...
{
	uli d, i, j;
	inline constexpr QEntry(uli d, uli i, uli j) : d(d), i(i), j(j) {}
};
...
void go()
{
	...
	const auto comp = [](const QEntry& lhs, const QEntry rhs) -> bi
	{
		return lhs.d > rhs.d;
	};

	priority_queue<QEntry, vector<QEntry>, decltype(comp)> Q(comp);
	loop(0, n, j)
	{
		loop(0, j, i)
		{
			Node ni = ds.nodes[i], nj = ds.nodes[j];
			uli dist = abs(ni.x - nj.x) + abs(ni.y - nj.y);
			Q.emplace(dist, i, j);
		}
	}
	...
	while (ds.k > 1)
	{
		QEntry next = Q.top();
		Q.pop();
		if (ds.Find(next.i) != ds.Find(next.j))
		{
			val += next.d;
			ds.Union(next.i, next.j);
		}
	}
	cout << val * 2;
}
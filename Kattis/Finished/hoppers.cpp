...
struct DisjointSetForest
{
	...
	inline void operator()(uli x, uli y)
	{
		x = (*this)[x];
		y = (*this)[y];
		if (x == y)
	...
	inline vuli Representatives()
	{
		vuli reps(k);
		for (uli x = 0, i = 0; i < k; x++)
		{
			if (x == P[x])
			{
				reps[i] = x;
...
int main()
{
	...
	// Detect odd cycles
	bool oddCycle = false;
	vuli parity(N, MAX(uli));
	queue<pair<uli, bool>> q;
	for (uli rep : dsf.Representatives())
	{
		q.emplace(rep, 0);
		while (!q.empty())
		{
			uli x = q.front().first, p = q.front().second;
			q.pop();
			if (parity[x] == MAX(uli))
			{
				parity[x] = p;

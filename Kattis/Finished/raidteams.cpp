...
struct Person
{
	string name;
	array<uli, 3> s;
	friend istream& operator>>(istream& strm, Person& p)
	{
		return strm >> p.name >> p.s[0] >> p.s[1] >> p.s[2];
	}
};
...
void go()
{
	...
	loop(0, N, i) cin >> people[i];
	array<vuli, 3> As;
	loop(0, 3, i)
	{
		As[i].resize(N);
		iota(ALL(As[i]), 0);
		sort(ALL(As[i]), [i, &people](uli l, uli r) -> bool
			{
				const Person& pl = people[l];
				const Person& pr = people[r];
				return pl.s[i] > pr.s[i] || pl.s[i] == pr.s[i] && pl.name < pr.name;
			});
	}
	vbi available(N, true);
	array<uli, 3> sidx{ 0, 0, 0 };
	vector<string> names;
	names.reserve(3);
	loop(0, N / 3, i)
	{
		...
		loop(0, 3, j)
		{
			const vuli& A = As[j];
			uli& idx = sidx[j];
			while (!available[A[idx]]) idx++;
			available[A[idx]] = false;
			names.emplace_back(move(people[A[idx]].name));
		}
		sort(ALL(names));
		loop(0, 3, j)
		{

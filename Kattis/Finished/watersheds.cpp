...
//{
//	inline constexpr size_t operator()(const pair<T, U>& x) const noexcept
//	{
...
template<>
struct std::hash<puli>
{
	inline constexpr size_t operator()(puli x) const noexcept
	{
...
void go()
{
	...
	cin >> H >> W;
	vvuli A(H, vuli(W));
	loop(0, H, i) loop(0, W, j) cin >> A[i][j];
	vvpuli P(H, vpuli(W));
	vvuli R(H, vuli(W));
	loop(0, H, i) loop(0, W, j) P[i][j] = { i, j };
	loop(0, H, i) loop(0, W, j) R[i][j] = 0;
	const auto find = [&P](puli x) -> puli
	{
		while (P[AT(x)] != x)
		{
			puli& p = P[AT(x)];
			x = p = P[p.first][p.second];
		}
		return x;
	};
	const auto merge = [&P, &R, &find](puli x, puli y) -> void
	{
		x = find(x);
		y = find(y);
		if (x == y) return;
	...
	const auto pred = [&A](puli x, puli y) -> bool
	{
	...
	{
		adj.clear();
		if (i > 0) adj.emplace_back(i - 1, j);
		if (j > 0) adj.emplace_back(i, j - 1);
		if (j < W - 1) adj.emplace_back(i, j + 1);
		if (i < H - 1) adj.emplace_back(i + 1, j);
		if (adj.empty()) continue;
		puli b = *std::min_element(ALL(adj), pred);
		if (A[i][j] > A[AT(b)])
			merge({ i, j }, b);
	}
	uli ch = 'a';
	std::unordered_map<puli, char> basins;
	loop(0, H, i) loop(0, W, j)
	{
		puli rep = find({ i, j });
		if (basins.find(rep) == basins.cend())
			basins.emplace(rep, ch++);
	}
	loop(0, H, i)
	{
		cout << endl;

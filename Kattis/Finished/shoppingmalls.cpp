...
{
	lli x, y, z;
	friend inline istream& operator>>(istream& l, Vec3& r)
	{
		return l >> r.z >> r.x >> r.y;
	}
	friend ldf dist(const Vec3& l, const Vec3& r)
	{
		lli dx = r.x - l.x, dy = r.y - l.y, dz = 5 * (r.z - l.z);
...
void go()
{
	uli N, M;
	cin >> N >> M;
	vector<Vec3> places(N);
	loop(0, N, i) cin >> places[i];
	vector<vector<pair<uli, ldf>>> graph(N);
	loop(0, M, i)
	{
		uli v, w;
		cin >> v >> w;
		string code;
		cin >> code;
		Vec3 a = places[v], b = places[w];
		ldf d = dist(a, b);
		if (code == "walking" || code == "stairs")
		{
		...
		{
			graph[v].emplace_back(w, 1);
			graph[w].emplace_back(v, 1);
		}
		else
		{
			graph[v].emplace_back(w, 1);
			graph[w].emplace_back(v, d * 3);
		}
	}
	...
	const auto pred = [&D](uli l, uli r) -> bool
	{
		return D[l] > D[r];
	};

	typedef priority_queue<uli, vuli, decltype(pred)> queue;

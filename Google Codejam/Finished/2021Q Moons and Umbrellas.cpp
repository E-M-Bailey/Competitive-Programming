...
inline constexpr li cost(char c1, char c2, li X, li Y)
{
	if (c1 == '\0' || c2 == '\0' || c1 == c2)
		return 0;
	return c1 == 'C' ? X : Y;
...
inline constexpr li minCost(uli n, char b, char a, li X, li Y)
{
	li bC = cost(b, 'C', X, Y), bJ = cost(b, 'J', X, Y), Ca = cost('C', a, X, Y), Ja = cost('J', a, X, Y);
	li eJa = n & 1 ? Ca : Ja, eCa = n & 1 ? Ja : Ca;
	li CCCC = bC + Ca;
	li JJJJ = bJ + Ja;
	if (n == 1)
		return min(CCCC, JJJJ);
	li CCCJ = bC + X + Ja;
	li JJJC = bJ + Y + Ca;
	if (n == 2)
		return min({ CCCC, JJJJ, CCCJ, JJJC });
	li CJCJ = bC + n / 2 * X + (n - 1) / 2 * Y + eJa;
	li JCJC = bJ + n / 2 * Y + (n - 1) / 2 * X + eCa;
	li CCJC = bC + (n - 1) / 2 * X + (n - 2) / 2 * Y + eCa;//
	li JJCJ = bJ + (n - 1) / 2 * Y + (n - 2) / 2 * X + eJa;//
	//li CJJC = bC + (n - 2) / 2 * Y + (n - 2) / 2 * X + cost(evenC, a, X, Y);
	//li JCCJ = bJ + (n - 2) / 2 * X + (n - 2) / 2 * Y + cost(evenJ, a, X, Y);
	return min({ CCCC, JJJJ, CCCJ, JJJC, CJCJ, JCJC, CCJC, JJCJ });
}
...
int main()
{
	...
	loop(0, T, t)
	{
		...
		loop(0, n, i)
		{
			char prev = i == 0 || S[i - 1] == '?' ? '\0' : S[i - 1];
			char cur = S[i];
			result += cost(prev, cur == '?' ? '\0' : cur, X, Y);
			if (cur == '?' && first == MAX(uli))
				first = i;
			else if (cur != '?' && first != MAX(uli))
			{
				result += minCost(i - first, first == 0 ? '\0' : S[first - 1], cur, X, Y);
				first = MAX(uli);
			}
		}
		if (first != MAX(uli))
			result += minCost(n - first, first == 0 ? '\0' : S[first - 1], '\0', X, Y);
